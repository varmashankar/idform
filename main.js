...
document.addEventListener('DOMContentLoaded', function () {
    ...
    class NotificationSystem {
        ...
        createNotificationCenter() {
            ...
            <div class="notification-list" id="notification-list">
                <div class="no-notifications">No new notifications. <br> Please use feedback form for request or to report any issue.</div>
            </div>
    ...
    // =========================
    // Section: Calculator Initialization
    // =========================
    // =========================
    // Section: Calculator Initialization
    // =========================
    function initializeCalculators() {
        ...
        if (pricingCalculator) {
            pricingCalculator.addEventListener("submit", function (event) {
                ...
                // Get elements right when the button is clicked
                ...
                if (!unitPriceElement || !codeElement || !resultElement) {
                    console.error("Critical Error: Alpha Calculator elements not found in the DOM.");
                    return;
                }

                const unitPrice = parseFloat(unitPriceElement.value);
                const codeInput = codeElement.value;
                const codeToPercentage = {
                    'p': 50, 'q': 55, 'r': 60, 's': 65, 't': 70, 'u': 75, 'v': 80,
                    'w': 85, 'x': 90, 'y': 95, 'z': 100, 'P': 50, 'Q': 55, 'R': 60,
                    'S': 65, 'T': 70, 'U': 75, 'V': 80, 'W': 85, 'X': 90, 'Y': 95, 'Z': 100,
                };
...
function clearFormGol() {
    ...
    function initializeDescriptionGenerator() {
        ...
        if (descGenerator) {
            ...
            document.getElementById('clearDesc').addEventListener('click', clearForm);
        }

        function generateDescription() {
            const userInput = document.getElementById('itemCode').value;
            const parts = userInput.split('|');
            const generateDescField = document.getElementById('generateDesc'); // Get reference to the output field
            ...
            if (parts.length === 2) {
                ...
                // Generate the final description string
                ...
                generateDescField.value = 'Invalid input format. Please use "Small Description | Item Code" format.';
            }

            // --- RELIABLE SELECTION LOGIC ---
            // Ensure the element is focused
            generateDescField.focus();
            // Use setSelectionRange for robust, cross-browser text highlighting
            generateDescField.setSelectionRange(0, generateDescField.value.length);
        }

        function clearForm() {
            document.getElementById('descGenerator').reset();
        ...
        if (feedbackForm) {
            feedbackForm.addEventListener('submit', async function (event) {
                ...
                try {
                    ...
                    if (responseMessage) {
                        responseMessage.innerHTML = '<i class="fas fa-check-circle"></i> Thank you! Your feedback has been submitted successfully.';
                    ...
                    // Show error message
                    if (responseMessage) {
                        responseMessage.innerHTML = '<i class="fas fa-exclamation-circle"></i> Sorry, there was an error submitting your feedback. Please try again.';
    ...
    // Initialize copy features when DOM is ready
...
saveUsernameBtn.addEventListener('click', () => {
    // GET A FRESH REFERENCE to the input field right now.
    const freshUsernameInput = document.getElementById('username-input');
    
    // Check if we found the input field before using it
    if (!freshUsernameInput) {
        console.error("CRITICAL: Could not find the username-input field in the modal.");
        alert("A page error occurred. Please refresh.");
        return;
    }

    // Now, use the fresh reference to get the value
    const username = freshUsernameInput.value.trim();
    ...
    if (username) {
        if (setUsername(username)) {
            ...
            // We need to call the close function from here
            const usernameModal = document.getElementById('username-modal');
            if (usernameModal) {
                usernameModal.style.display = 'none';
            }
